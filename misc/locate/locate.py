...
class Actions:
    ...
    def locate_grab_line(direction: str) -> tuple[bool, int, int]:
        """Locate and grab line"""
        ok, x, y = actions.user.locate_hover(f"line_{direction}.png")
        if ok:
    ...
    def locate_line_move(direction: int):
        """Move line"""
        ok, x, y = actions.user.locate_grab_line("vertical")
        if ok:
            actions.sleep(0.2)
